<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" class="listTable" fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="300px" label="分类">
        <template slot-scope="scope">
          <span>{{ scope.row.property_category.title }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <router-link class="mr-10" :to="{name:'PropertyEdit',params:{id:scope.row.id}}">
...
<script>
import { fetchProperties, deleteProperty } from '@/api/product'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const response = await fetchProperties(this.listQuery)
